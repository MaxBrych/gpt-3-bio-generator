      ...
      <div className="hidden h-full gap-2 px-4 py-4 bg-white w- md:flex md:flex-col md:gap-4 ">
        {tools.map((tool) => (
          <Link
            ...
            as={`/tools/${tool.slug}`}
            key={tool.id}
            className={`flex items-center justify-start gap-2 p-2 rounded-full cursor-pointer h-14 transition md:p-4 md:border-cyan-90 md:hover:bg-cyan-95 ${
          ...
          >
            ...
            <h1 className="text-xs font-semibold leading-3 md:text-sm md:text-left">
              {tool.name}
            </h1>
          </Link>
        ))}
